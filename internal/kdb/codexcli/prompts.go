...
func BuildClassifyPrompt(ko string, spellings map[string]string, sourceDomains []string, notes string, wikidata *Wikidata, searchHits []string) string {
	...
	lines := []string{
		...
		"",
		"",
		"SCOPE FILTER (do this THIRD — KDB 는 **한국 대상** 고유명사 DB):",
		"- ★2026-09-15 범위 확대(운영자 결정). 종전엔 'K-엔터테인먼트 전용'이라 정치인·",
		"  운동선수·기업인·학자를 전부 범위 밖으로 버렸다. 그 결과 이재명(대통령)·차범근",
		"  (축구선수)·서울대학교가 기각돼 소비자 요청이 영구히 막혔다. 지금은 받는다.",
		"- 범위 안: **한국의** 인물(분야 무관 — 연예·정치·경제·스포츠·학계·언론),",
		"  한국의 작품, 한국의 조직·기관(정당·정부기관·기업·협회·구단·학교·방송사·소속사).",
		`- 범위 밖이면 entity_type="term", confidence ≤ 0.30, needs_search=false,`,
		`  reason="범위밖: <간단설명>" 로 분류한다(다운스트림이 reject).`,
		`- 범위 밖 예: **한국 대상이 아닌 것** — "젠슨 황"(NVIDIA CEO)·"팀 쿡"·"일론 머스크",`,
		`  "안젤리나 졸리"·"숀 펜"(외국 배우), "Bad Bunny"(외국 가수), 외국 정치인·운동선수,`,
		"  그리고 제품·서비스명(갤럭시 S25), 일반 경제용어(기준금리), 일반 명사, 문장.",
		"- 예외(범위 안 = person 정상): ①K-pop 그룹의 외국국적 멤버(예: NewJeans 하니/다니엘,",
		"  (G)I-DLE 우기·민니·슈화, aespa 닝닝, 트와이스 모모/사나/미나), ②한국에서",
		"  활동하는 외국인(가수·배우·방송인·선수·교수 등). 활동 무대가 한국이면 범위 안이다.",
		"",
		"- Otherwise proceed with classification below.",
		"",
		"CONSUMER TYPE HINT (2026-07-25 인입감사 — 실측 11% 타입 불일치, 절반이 힌트가 옳았음):",
		"- Existing notes 에 '소비자 type힌트=<type>' 이 있으면 그 힌트는 요청 매체가 기사 문맥에서",
		"  준 강한 사전확률이다. 검색/Wikidata 에서 *명확한 반대 증거* 가 없는 한 힌트 type 을 따르라.",
		"- 힌트를 뒤집을 때는 reason 에 반대 증거를 명시하라(막연한 추정으로 뒤집기 금지 —",
		"  예: 윤하 곡 '사건의 지평선'을 이름 느낌만으로 brand_place 로 뒤집은 실사고).",
		"",
