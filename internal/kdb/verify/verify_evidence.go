...
func buildVerifyPrompt(vi verifyInput) string {
	e := vi.e
	var b strings.Builder
	// ★범위를 0143 에 맞춘다 (2026-09-20). 이 프롬프트만 옛 세상에 남아 있었다.
	//
	//   게이트(gatekeeper/intake.go)는 2026-09-15 에 v4 로 올려 «한국의 인물·작품·
	//   조직·기관» 을 받게 고쳤다. 그런데 **이 판별기는 안 고쳤다.** 그래서 새 유형이
	//   문을 통과해 들어온 뒤, 여기서 «K-콘텐츠가 아니다»로 다시 죽었다.
	//
	//   특히 아래 2번 규칙에 «의약품·스포츠·정치 등 무관 분야» 가 그대로 있었다.
	//   스포츠와 정치는 0143 이 **명시적으로 넣은** 분야다. 판별기가 그것을 오염으로
	//   보고 있었다.
	//
	//   실측(2026-09-20): [cand-evidence:review] 1,623행 · 그중 옛 범위 문구가 근거인
	//   것 1,133행(70%). 그리고 **지금도 하루 100~180건씩 새로 찍히고 있었다**
	//   (9/16:182 · 9/17:144 · 9/18:118 · 9/20:146). 막힌 502 candidate 중 147 이
	//   company·organization·school·government_body·political_party·sports_team —
	//   정확히 0143 이 추가한 유형들이다.
	//
	//   소비자가 지금 이 순간 묻고 있는 것들이 여기 걸려 있었다:
	//     농협     "한국 대중문화 엔티티가 아닌…"
	//     롯데마트 "롯데마트는 한국 대중문화(K-콘텐츠) 엔티티가 아닌 유통…"
	//     이마트   "이마트는 한국 대중문화(K-콘텐츠) 엔티티가 아닌…"
	//
	// ★범위가 넓어졌다고 «아무거나 통과»가 아니다. 밖에 남는 것은 그대로다 —
	//   해외 대상 · 일반 명사 · 광고/상품 · 저장 유형이 근본적으로 틀린 것.
	b.WriteString("당신은 한국 고유명사(인물·작품·조직·기관)의 '기사맥락 판별기'입니다.\n")
	...
	b.WriteString("\n판별 규칙(기사 맥락 기준, 엄격):\n")
	b.WriteString("★핵심: '실존하는 한국 대상인가'만 본다. 연예·문화뿐 아니라 정치·경제·시사·스포츠·학술·행정도 범위 안이다 — 국회의원·운동선수·기업·대학·정부기관·정당 전부 포함. 세부 역할/대표작이 우리 DB와 달라도 실존하면 real 이다(메타데이터는 나중에 보강). 기각(contaminated)은 아예 한국 대상이 아닐 때만.\n")
	...
	b.WriteString("1. verdict=real: 뉴스가 이 이름을 실존하는 한국 " + e.etype + "(인물·그룹·작품·기업·기관·단체 등)로 뒷받침. 우리 DB의 역할/작품과 세부가 달라도 실존하는 한국 " + e.etype + "면 real. identity=기사가 말하는 정체를 한 구절로.\n")
	b.WriteString("2. verdict=contaminated: 이 이름이 '한국 대상이 아예 아님' — 해외 인물·해외 기업, 일반 단어/명사, 광고·상품명, 또는 저장된 종류(" + e.etype + ")가 근본적으로 틀림(예: 넷플릭스 다큐멘터리인데 노래·앨범으로 저장). identity=기사가 말하는 실제 정체.\n")
