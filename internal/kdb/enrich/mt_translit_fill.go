...
func judgeTranslit(ctx context.Context, ko, mt, locale string) (kind, reason string) {
...
SELECT id::text FROM kwave_entities
 ...
 LIMIT $1`, limit)
	...
	for _, idStr := range ids {
		...
		if mt == "" { // 미번역(구글이 모르는 고유명사) — 버림
			discarded++
			continue
		}
		kind, reason := judgeTranslit(ctx, snap.Ko, mt, locale)
		if kind != "translit" {
			discarded++
			if dry {
		...
		if len(applied) > 0 {
			...
			discarded++ // 문자셋/suppress 가드 기각 또는 동시 채움
		}
	}
