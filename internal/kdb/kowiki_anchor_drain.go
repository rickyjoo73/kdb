...
var koWikiCtxRe = regexp.MustCompile(`(?i)대한민국|한국|남한|` +
	...
	// ★0143 유형의 신호를 더한다 (2026-09-21). 이 목록이 K-웨이브 어휘뿐이라
	//   기업·정당·대학·구단·기관 문서는 «한국 맥락 없음» 으로 떨어졌다. 이 파일의
	//   원칙대로 **긍정 신호를 넓히는 쪽**으로 고친다(부정 목록을 늘리지 않는다).
	//   나라를 가리지 않는 장르어(드라마·영화제)를 뺐던 이유와 어긋나지 않는다 —
	//   아래는 전부 한국 제도·행정 어휘다.
	`대학교|대학원|고등학교|중학교|초등학교|` +
	`정당|국회|의회|시청|군청|도청|교육청|구청|` +
	`부처|정부|공공기관|공사\b|공단|위원회|` +
	`협회|재단|학회|조합|연합회|` +
	`프로야구|프로축구|구단|KBO|K리그|` +
	`주식회사|기업|은행|증권`)
...
func koWikiVerdict(ko string, p *koWikiPage, known *koWikiNameSet) (ok bool, verdict, reason string) {
	...
	return false, "foreign-topic", "도입부에 한국 대상 맥락 없음: " + firstRunes(p.Extract, 60)
}
