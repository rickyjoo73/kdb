	...
	return []workflowTrackSpec{
		...
		},
		{
			...
			Steps: []workflowStepSpec{
				...
				{Num: "B10", Name: "현지표기 검색보강(LocalFill)", Detail: "KDB 자체(SearXNG 메타검색)+gemma 다회투표 → 2단계 확정-승급(약→local-search 빈칸, 강=만장일치+grounding→local-usage tier1) · 재그라운딩(LLM 합성값 교체). LLM 은 gemma 하나다(codex 폐기 2026-09-16)", Agent: "LocalFill",
					LLMUsed: true, ProviderLiteral: "gemma", Supervised: true, MetricKey: "localusage"},
				{Num: "B11", Name: "cascade L2/L3", Detail: "MusicBrainz/TMDb/KOFIC/Wikidata 채움", Agent: "SourceFetch"},
				{Num: "B12", Name: "cascade L4", Detail: "빈칸 LLM 합성(gemma · 출처키는 codex-fallback 라는 옛 이름을 쓴다)", Agent: "Enricher",
					LLMUsed: true, ProviderKey: "FILL", ProviderDef: "gemma", EffortKey: "FILL", EffortDef: "medium", Supervised: true, MetricKey: "backlog"},
				{Num: "B13", Name: "fallback 검증·승급", Detail: "LLM 합성값 → Wikidata QID 라벨로 재검증·승급(QID 보유분). QID 없는 사각지대는 B10 재그라운딩이 담당", Agent: "FillVerifier",
