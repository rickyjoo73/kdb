...
## 16. 현행 카탈로그 대조와 P0.02 결정 — 2026-09-13

운영 KDB 카탈로그와 §4~§15 를 표 단위로 대조한 결과는 [KDB_SCHEMA_DIFF_0115_0122.md](KDB_SCHEMA_DIFF_0115_0122.md)에 있다.
설계 결정을 바꾼 것은 없고, 설계가 전제한 현행과 실제가 다른 지점 15건(D-01~D-15)을 P1 시험 항목으로 지정했다.

P0.02 에서 추가로 결정한 컬럼 하나: `kentity_entities.qualifier_ko`(text ?, NULL). legacy `kwave_entities.disambig` 와
TDB `tdb_places.disambiguator` 의 공통 호환 자리다. 표시용 한정어이며 정체성 키·검색 UNIQUE·동명 판정 근거가 아니다.
동명 구별의 근거는 §11 프로필과 §10.6 관계가 맡는다. 이 컬럼이 있다는 이유로 동명이인 판정을 자동화하지 않는다.

§9 의 6묶음은 다음 위치로 닫힌다: 그룹1 → 위 문서 §5 + §11, 그룹2 → §10 + 위 문서 §6 seed, 그룹3 → §12 + §14.3/14.8 + seed,
그룹4 → 위 문서 §2·§4, 그룹5 → 위 문서 §7 + 제어 설계, 그룹6 → 위 문서 §4 D-항목. 역할 분리는 쓰기 권한 조사의 §물리 권한 매트릭스에 있다.
P0.05 의 pending 7표 판정이 이 대조에 행을 추가할 수 있으나 표 구조를 바꾸지는 않는다.
