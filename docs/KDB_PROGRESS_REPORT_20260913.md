...
[실행 TODO](KDB_INTEGRATION_TODO.md) 66개 중 완료 체크는 4개, 미완료는 62개다. (후속 §7~§13: 같은 날 P0.06·P0.02·P0.05·P0.08·P0.09·P0.10·P1.01 인수로 11개/55개. G0 인수.)
...
## 12. 후속 — 2026-09-13 P0.10 모순 검토·G0 인수

- 확인 근거: 설계 문서 9종 교차 검토(구조·코드표·매핑·UI·테스트·선행 검토), 운영 카탈로그·Go 허용값 대조. DDL·데이터 변경 없음.
- 완료: P0.10. **P0 전체 10/10.** 해결 결정 4, 미결 보류 12(전부 P1 이후·차단/제외 명시), 승인 범위 = 파일럿 A/B.
- **G0 인수** — 통과 기준(승인 범위 필드·키·FK·NULL·책임 정의 완료 + 미결 차단/제외 명시) 충족. 되돌리려면 원장 §8 P0.10 기록 한 줄.
- 운영 변경·배포: 없음. 커밋·푸시: 이 후속 커밋.
- 다음: **P1.01** — 승인된 격리 DB 에 최신 백업 복원, 역할/스키마/이미지 재확인, 0115~0122 재사용·보강·회귀 대상 식별. 이어서 P1.02 forward migration 초안(D-01/03/07/11/12/13 + btree_gist).

## 13. 후속 — 2026-09-13 P1.01 격리 복원·기준선 인수

- 확인 근거: 운영 READ ONLY 재확인 + 격리 컨테이너 `kdb-p1-restore-db` 에 12:00 백업 복원. 운영 DDL·데이터 변경 없음.
- 완료: P1.01. 복원 ERROR 0·구조 10/10 일치·행 손실 0. D-01(evidence 중복 0)·D-03(고아 UUID 0)·D-05(btree_gist 1.7, EXCLUDE 6/6 — 동명이인 같은 직업 허용 실증)·D-07/11/12/13 해소. 트리거 16 분류(회귀 4·보강 4·재사용 8).
