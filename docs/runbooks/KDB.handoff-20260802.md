...
### 아직 안 한 기제 2·3 (오너 결정 대기)

- **기제2 · 근거 원장 필수화 + 스니펫 저장** — `5ed2af4` 가 URL 은 저장하나 스니펫이 없어
  게이트 결정을 재현할 수 없다(§8 자기정정의 원인). 근거를 원장에 못 남기면 승급이
  성립하지 않게 한다. 비용 낮음~중간. **이걸 해야 `evidenced-unretrievable` 3,995 가
  신규 유입분부터 실제로 내려간다.**
