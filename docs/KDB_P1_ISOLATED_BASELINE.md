...
# P1.04 / P1.05 / P1.06 / P1.09 — 인수 사례 완주와 검사, 2026-09-13

## 39. M01~M14 어디까지 왔나

| 사례 | 무엇을 보나 | 어디서 통과했나 |
|---|---|---|
| M01 | 동명 가수/배우가 각자 직업·근거를 가진다 | P1.03 SQL |
| M02 | 이름·직업·생년이 같아도 자동 병합하지 않는다 | P1.03 SQL |
| M03 | 겸업은 UUID 하나로 두 필터에 잡힌다 | P1.03 SQL |
| M04 | 같은 명칭의 회사·장소·상품·인물이 각자 UUID | P1.03 SQL |
| M05 | 역/상점, 학교/캠퍼스가 별개 UUID | P1.03 SQL |
| **M06** | **문맥 없는 이름 → ambiguous + 후보** | **P1.08 API** |
| **M07** | **한 기사의 동명 span 2개가 각자 UUID** | **P1.08 API + S01 SQL 7건** |
| M08 | 같은 QID 가 두 유형에 verified 로 | P1.03 SQL |
| M09 | 개명·직업 추가·이적은 기존 UUID 의 변경 | P1.03 SQL |
| **M10** | **원본 재수집·배치 재실행에도 대상 UUID 가 흔들리지 않는다** | **P1.05 SQL 6건** |
| M11 | unknown 유형·직업 없는 인물 | P1.03 SQL |
| M12 | 정정/철회 guard | P1.03 SQL 11건 |
| M13 | 늦은 job·동시 writer·tenant scope | P1.03 SQL |
| M14 | locale/형식/정책 승인 | P1.03 SQL 8건 |

M10 의 요지는 "이름으로 다시 풀어서 다른 UUID 에 붙이지 않는다"이다. 이름은 키가 아니므로
(I01) 같은 원본을 두 번 가져와도 연결은 1건이어야 하고 대상 UUID 는 그대로여야 한다.
연결의 3컬럼 PK, 같은 run 안의 중복 계상, `create` 의 기대 revision, `applied` 의 계상 수를 건다.

## 40. P1.06 — 두 세션이 실제로 겹칠 때

기존 P1.06 시험 5건은 전부 단일 세션의 제약 검사였다(멱등키·기대 revision·승인 없는 apply·
감사 모양). 병합에서 실제로 위험한 것은 **두 세션이 같은 대상을 동시에 건드릴 때**이고,
그건 SQL 한 문장으로 만들 수 없다. Go 동시성 시험 4건을 더했다.

| 시험 | 결과 |
|---|---|
| 같은 외부 ID 를 두 대상이 동시에 요청 | 하나만 성공 + 예약에 주인 기록 — **처음엔 둘 다 성공했다(D-35)** |
| 양방향 병합 요청 동시 실행 | 교착 없음 (UUID 순서 잠금) |
| 직렬화 충돌 | `40001` 이 호출자에게 그대로 올라온다 |
| manifest 중간 실패 | 앞 문장도 남지 않는다 (I10) |

### D-35 — 조건은 있는데 값이 없었다

`kentity_reserve_legacy_external_id()` 는 `owner_id IS NOT NULL AND owner_id <> NEW.entity_id`
일 때 거부한다. 그런데 예약을 만들 때 `entity_id` 를 채우지 않아 **새 예약의 주인이 항상
NULL** 이었다. 조건문은 있는데 그 조건이 참이 될 수 없으니 영원히 통과한다. 격리본에서
서로 다른 두 legacy 대상이 같은 wikidata QID 를 동시에 가져가는 것을 재현했다.
식별 계약 §4 "검증된 한 외부 키의 현재 소유자는 하나다"가 이 경로에서만 비어 있었다.

`DO UPDATE` 로 기존 행을 잠가 동시 요청을 직렬화하고, 먼저 온 주인은 `COALESCE` 로 유지한다.

## 41. P1.09 — 기존 API replay

이 저장소에서 실행 가능한 것은 7건이고 6건을 돌렸다. R03/R04/R09/R10/R11 은 TDB 트리가 필요하다.

| R | 내용 | 결과 |
|---|---|---|
| R01 | 오류 봉투가 status/code/message 를 보존 | 통과 (서버 계약 유지, client 쪽 유실은 client 문제) |
| R02 | `/api/health` 와 `/api/v1/health` | **고침** — 후자가 `/v1/v1/health` 가 돼 404 였다. 한 번만 정규화 |
| R05 | route 별 locale 규칙 matrix | 통과 (match 정규화·미지원 400 / legacy prepare 미거부) |
| R06 | terms 201 → cap 200 | 통과. accepted/truncated 노출은 응답 모양 변경이라 P1.10 |
| R07 | 멱등키·owner·revision | 통과 (기존 preparations 시험이 이미 커버) |
| R12 | 깨진 델타 커서 | **고침** — 조용히 무시하고 전체 조회로 진행했다. 400 으로 돌려준다 |
| R08 | 교정 쓰기 + 감사 원자성 | 정리 계약 필요 → P1.10 |

R02·R12 를 지금 고친 근거: **운영 요청 로그 98,489건 실측으로 둘 다 사용 0건.** 실사용 회귀
없이 목표 불변조건을 만족시킬 수 있는 경우다. 특히 R12 는 깨진 커서를 200 으로 받아 전건을
"변경분"으로 돌려주던 것이라, 커서가 깨졌다는 사실만 사라지고 잘못된 의미가 남았다.

## 42. P1.09 — 권한/CSRF

기존 `csrf_test.go` 는 미들웨어를 단독 호출할 뿐 라우터를 통과시키지 않았다. 그래서
"POST 라우트를 인증 그룹 **밖에** 추가한다"거나 "`r.Use(s.csrfProtect)` 를 지운다"는 회귀를
잡지 못했다. `chi.Walk` 로 등록부를 직접 걸어 전수로 확인한다.

- 인증 그룹 POST **37개 전부** 토큰 없는 요청을 403 으로 거부.
- 면제는 `/admin/login`·`/admin/setup` 둘뿐이며, 목록이 조용히 늘면 시험이 깨진다.
- 운영자 전용 서브트리 POST 11개가 `readinessStaffAuth` 아래 그대로 있는지 확인.

손으로 적은 목록을 쓰지 않은 이유가 있다 — `/admin/entities/homonyms` 는 핸들러만 있고
등록이 빠져 404 로 죽어 있었는데, 손으로 적은 목록에는 애초에 없었으니 아무도 눈치채지 못했다.

## 43. P1.09 — 390px / 1440px

브라우저 없이 픽셀을 잴 수는 없다. 대신 **깨지는 원인**을 정적으로 잡는다.

| 원인 | 실측 | 처리 |
|---|---|---|
| 가로 스크롤 래퍼 없는 표 | **9개** (그중 5개는 `overflow-hidden` 안이라 스크롤도 안 되고 잘렸다) | 전부 감쌌다 |
| 반응형 없는 3열 이상 격자 | 1개 (`api_requests.html`) | `grid-cols-1 sm:grid-cols-3` |
| 접히지 않는 고정 사이드바 | 1개 (`entity_homonyms.html`, 390px 에서 본문에 150px 만 남김) | 좁은 화면에서 접음 |

`partials.html` 은 `#admin-navigation` 을 `max-width:767px` 에서 감추고 토글을 띄운다 —
Tailwind 유틸이 아니라 media query 로 하는 정상 구현이라 판정에서 통과시킨다.

## 44. P1.09 — 조회 성능 (복원본 19,516 엔티티)

| 경로 | 실측 | 상한 |
|---|---|---|
| match (기사 본문 1,000자급) | **614ms** | 3s |
| match (문맥 없는 이름) | **413ms** | 2s |
| entities 목록 + `updated_since` | **13ms** | 2s |
| entities 부분검색 | **270ms** | 2s |
| 동명 그룹 집계 (화면 전체) | **32ms** | 2s |

2026-08-04 에 match p99 8.6s 를 찍어 판별 예산을 남은 시간 기준으로 바꾼 전력이 있으므로,
이 경로는 회귀 감지를 상시로 둔다.

## 45. D-36 — 두 규칙을 합치면 자동 병합 경로가 도달 불가가 된다

D-35 를 고치자 **기존 병합 시험 2건이 깨졌다.** 원인을 따라가 보니 시험의 문제가 아니라
설계 두 조각이 서로를 막고 있었다.

| 규칙 | 내용 | 출처 |
|---|---|---|
| S04 | "서로 다른 UUID가 같은 (provider,external_id)를 reserve/verify할 수 없다" | 구조 §14.4 |
| 자동 병합 게이트 | `loser.qid != winner.qid` 면 거부 — "공유하는 안정 식별자 없이 이름·직업만으로 병합을 승인하지 않는다" | `internal/kdb/agents/disambiguator/merge.go:145` |

둘 다 맞는 말이다. 그런데 함께 두면 **자동 병합이 요구하는 상태(두 UUID 가 같은 QID 보유)를
애초에 만들 수 없다.** 지금까지 그 상태가 만들어진 것은 예약이 주인을 기록하지 않아서였다(D-35).
즉 자동 병합은 **버그 덕분에 동작하고 있었다.**

**해소 방향(P5):** 동일인 판정은 "둘 다 같은 외부 ID 를 들고 있는가"가 아니라 검수된
`kentity_resolutions` 의 `confirmed_same` 이어야 한다. 식별 계약 §4 가 이미 그렇게 규정한다 —
"판정은 정렬된 UUID 쌍에 대해 possible_same/confirmed_same/distinct/withdrawn 을 기록한다."
legacy 자동 병합은 그 판정 경로로 대체된다. 이건 실사용 writer 전환이라 P5 다.

**P1 에서 한 것:** 제약은 그대로 둔다(S04 가 안전 쪽이다). 병합 시험은 "제약 이전에 쌓인
중복 상태"를 명시적으로 재현하도록 바꿨다 — 운영 데이터에는 그런 행이 실제로 남아 있고,
자동 병합은 그것들을 소화하는 동안만 의미가 있다.

## 46. 시험 격리에서 드러난 것 둘

**하드코딩 픽스처가 서로의 예약을 뺏고 있었다.** `readiness` 와 `disambiguator` 의 복원 시험이
둘 다 `Q123456` 을 쓴다. 같은 복원 DB 를 공유하므로 먼저 도는 쪽이 예약을 가져가고 나중 쪽이
거부된다. 지금까지는 예약이 주인을 기록하지 않아 아무나 통과했을 뿐이다. UUID 에서 결정적으로
만드는 고유 QID 로 바꿨다.

**시험 픽스처가 회귀 DB 로 새고 있었다.** `p1_tests.sql` 은 마지막에 COMMIT 하므로 합성
Entity·연결·근거가 `kdb` 에 남는다. 그런데 회귀 DB 를 `TEMPLATE kdb` 로 만들던 순서가
**시험 뒤**라 픽스처가 통째로 복제됐다. `TestCommonCatalogAgainstRestoredSchema` 가
"confirmed 연결 0건"을 보는데 M10 이 만든 1건이 잡혀 실패했다. 회귀 DB 복제를 **구조 적용
직후·시험 전**으로 옮겼다 — 회귀 DB 는 "복원본 + P1.02 구조"여야 하고 시험 픽스처는 아니다.

---

# P1.10 — G1 인수, 2026-09-13

## 47. 게이트 조건 대조

> G1 통과: M01~M14의 DB/API 해당 범위와 필수 제약 통과, 동명 ID 사이의 표기/근거 교차 저장 0,
> 기존 보호/실사용 계약 회귀 없음.

### ① M01~M14 통과

14건 전부 격리 복원본에서 실행했다. 인수 픽스처(`docs/checks/kdb-acceptance-fixtures.json`)의
`execution_status` 를 `executed_p1_isolated` 로 바꾸고 사례마다 어느 시험이 근거인지 적었다.
검사기도 함께 바꿨다 — 허용값은 미실행/격리실행 둘뿐이고, **운영 실행을 주장하면 실패한다.**

### ② 동명 ID 사이의 교차 저장 — 실측 전부 0

| 검사 | 결과 |
|---|---|
| 이름이 다른 Entity 의 근거를 참조 | **0** |
| 직업이 다른 Entity 의 근거를 참조 | **0** |
| 분야가 다른 Entity 의 근거를 참조 | **0** |
| 외부 ID 가 다른 Entity 의 근거를 참조 | **0** |
| readiness 가 다른 Entity 의 이름을 참조 | **0** |
| readiness 가 item 의 bound 와 다른 대상 | **0** |
| 이름근거가 다른 Entity 를 가리킴 | **0** |
| 한 외부 키를 둘 이상이 예약 | **0** |

합성 픽스처 22건이 들어 있는 상태에서 잰 값이다. 교차 저장을 **시도한** 시험이 여러 건 있고
(M01b·M07b·M07c·TRG06c 등) 전부 거부됐기 때문에 0 이다.
...
| `go test ./...` | **31 ok / 0 FAIL** |
| `go test -race ./internal/...` | **29 ok / 0 FAIL** |
| 격리 복원 DB 회귀 | **5/5 패키지 ok** |
| SQL 제약·인수 시험 | **107/107 PASS** |
| 정적 검사 6종 | 전부 PASS |
| API replay | 6건 실행, 계약 유지 확인 |

### ④ 시험 전용 승인값이 운영에 들어가지 않는다

| 대상 | 승인 provider | 합성 Entity | 시험 전용 `prov-x` |
|---|---|---|---|
| 격리본(시험 후) | 14 | 22 | **있음** |
| **회귀 DB(구조만 적용)** | **13** | **0** | **없음** |
| 운영 | 정책 표 자체가 없음 | — | — |

승인 정책 13 provider 는 `p1_structure.sql` 의 seed(운영자 결정)이고, `prov-x` 는 `p1_tests.sql` 이
만든다. 구조 파일만 올리면 시험 전용 값은 따라가지 않는다 — 회귀 DB 를 **시험 전에** 복제해
실증했다. 운영은 표 60·정책 표 없음·`kdb-app healthy restarts=0` 로 무변경이다.

## 48. 설계에 반영한 실패 사례

P1 에서 격리 실행으로만 드러난 것이 **D-16 ~ D-36, 21건**이다. 문서 대조로는 하나도 안 보였다.

| 성격 | 건수 | 대표 |
|---|---|---|
| 트리거 본문이 옛 스키마를 가정 | 8 | D-16·D-23·D-24·D-25·D-30·D-31·D-32·D-33 |
| 제약이 있는데 값이 없어 도달 불가 | 2 | **D-35**(예약 주인 미기록), D-26(정책 증명 없는 ready) |
| 제약 자체의 구멍 | 4 | D-19(NULL 전파로 CHECK 뚫림)·D-21(UUID 가변)·D-34(provider 당 승인 복수)·D-29(이름+유형이 정체성 키) |
| 쓰기 순서·문장 단위 | 2 | D-27·D-28 |
| 설계 조각끼리의 모순 | 1 | **D-36**(S04 ↔ 자동 병합 게이트) |
| 기타 | 4 | D-17·D-18·D-20·D-22 |

가장 값진 것 셋:

- **D-30** — 안전장치가 안전장치를 막았다. 내가 P1.07 에서 넣은 이름 보호선 때문에, 운영자가
  잠근 이름이 하나라도 있으면 그 근거를 철회할 수 없었다. 보호선을 넣을 때 "무엇을 막을까"만
  보고 "무엇이 이 표를 정당하게 내리는가"를 보지 않았다.
- **D-35/D-36** — 조건문은 있는데 조건이 참이 될 수 없었고, 그걸 고치자 다른 규칙과 충돌이 드러났다.
  자동 병합이 **버그 덕분에** 동작하고 있었다는 뜻이다.
- **공회전하던 단언 12개** — 조건절만 쓰는 단언은 거짓이어도 0행일 뿐 오류가 아니다.
...
| 항목 | 왜 | 어디로 |
|---|---|---|
| D-29 이름+유형의 정체성 키 사용 | 운영 동작 변경 | P2.05(진입점)·P3(UNIQUE 이관) |
| D-36 자동 병합 → 판정 경로 전환 | 실사용 writer 전환 | P5 |
| R08 교정 쓰기 원자성 | 쓰기 경로 정리 계약 필요 | P1.10 이후 replay 확장 |
| R06 accepted/truncated 노출 | 응답 모양 변경 = 소비자 계약 | P5 |
| R03·R04·R09·R10·R11 | TDB 트리 필요 | P2 이후 |
| 감사 SUSPECT 5건(§38) | 미재현 | 시험 선행 후 판단, 유형 전환 건은 **P2.02 전** |
| 표기 위생의 DB 이관 | Go 함수 의존 | P5 |
| span linking | 픽스처가 P7 로 명시 | P7 |

## 50. G1 판정

**인수한다.** 게이트 조건 세 가지를 모두 실측으로 충족했고, 넘긴 항목은 전부 "다음 단계의 범위"
이거나 "운영 변경이라 승인이 필요한 것"으로 분리해 적었다. 운영 DB 는 P1 전체에서 무변경이다.

★ **배포 순서 주의는 유효하다.** Go 변경은 `p1_structure.sql` 스키마를 전제한다. 코드만 먼저
나가면 운영이 깨진다. P3.02 에서 구조와 코드를 한 배포로 묶는다.
