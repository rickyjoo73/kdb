-- P3 파일럿 B batch 1 — admin_region 121행 적재
--
...
SELECT '7f000001-0000-4000-8000-000000000001','operator','p3-pilot-b-batch-1',
       md5(md5('p3-pilot-b-batch-1'))||md5('v1'),'apply','running',
       ...
       md5(md5('admin_region'))||md5('cohort-b1'),
       md5(md5('tdb-selection-v1'))||md5('policy-b1'),
 ...
 WHERE NOT EXISTS (SELECT 1 FROM kentity_migration_runs WHERE id='7f000001-0000-4000-8000-000000000001');
...
SELECT '7f000001-0000-4000-8000-000000000001','tdb','tdb_places',
       ...
       CASE WHEN t.tdb_id IS NOT NULL THEN d.reason_code ELSE d.reason_code||'_not_in_batch_1' END
  FROM b1_decision d LEFT JOIN b1_target t ON t.tdb_id = d.tdb_id
 WHERE NOT EXISTS (SELECT 1 FROM kentity_migration_records r
                    WHERE r.run_id='7f000001-0000-4000-8000-000000000001'
...
SELECT 'tdb','tdb_places', t.tdb_id::text, t.new_entity_id, 'confirmed',
       ...
       'tdb-premap-v1',
       'P3 파일럿 B batch 1: QID 앵커와 행정 코드로 확정. 운영자 승인 범위.',
...
UPDATE kentity_migration_runs
   SET state='completed', finished_at=now(),
       expected_records=(SELECT count(*) FROM kentity_migration_records WHERE run_id='7f000001-0000-4000-8000-000000000001')
...
UNION ALL SELECT '기록(전량 계상)', count(*)::text FROM kentity_migration_records WHERE run_id='7f000001-0000-4000-8000-000000000001'
UNION ALL SELECT '그중 applied', count(*)::text FROM kentity_migration_records WHERE run_id='7f000001-0000-4000-8000-000000000001' AND state='applied';
