-- P1.02 격리 forward migration — 목표 구조(draft-v4)의 최소 차이
--
-- 적용 대상: 격리 복원본(kdb-p1-restore-db)뿐이다. 운영 DB 에 적용하지 않는다.
-- migrations/ 밖에 있으므로 deploy.yml 의 `ls migrations/*.sql` 글롭에 잡히지 않는다.
-- 운영 승격은 P3.02 승인 범위에서 별도 판단하며 그때 migrations/ 로 번호를 붙여 옮긴다.
--
-- 근거: docs/KDB_TARGET_SCHEMA.md §4~§15, docs/KDB_MIGRATION_CONTROL_SCHEMA.md,
--       docs/KDB_SCHEMA_DIFF_0115_0122.md §2~§4(D-01~D-15), docs/KDB_P1_ISOLATED_BASELINE.md §7.
-- 전부 한 트랜잭션이다. 하나라도 실패하면 전체 rollback 된다.
