...
## 0.5 `id` 를 무엇에 쓰는가 — **검색 키가 아니라 신뢰 표식**

기사에서 "김하늘"을 만난 사람이 ID 를 알 리가 없다. **ID 로 검색하라는 뜻이 아니다.**
ID 는 **답에 실려 나가는 신원 표식**이고, 그게 있으면 이런 것을 할 수 있다:

```json
{"ko":"채영","id":"84ab95a9-…","disambig":"(TWICE)","locale_name":"チェヨン"}
{"ko":"채영","id":"2dd14daf-…","disambig":"(CLC)",  "locale_name":"チェヨン"}
```

- **같은 사람인지 안다.** 기사 A 의 채영과 기사 B 의 채영이 같은 `id` 면 같은 사람이다.
  일본어 표기가 `チェヨン` 으로 똑같아도 `id` 가 다르면 **다른 사람**이다.
  이름만 보고는 절대 알 수 없는 것을 `id` 가 말해 준다.
- **용어집에 못 박을 수 있다.** 기사 안에서 한 번 고른 `id` 를 계속 쓰면 표기가 흔들리지 않는다.
- **정정을 그 대상에 신고할 수 있다.** `/v1/corrections` 는 `entity_id` 를 받는다.

**동명이인은 전부 돌려준다. 우리가 하나를 고르지 않는다**(M06).
고르는 데 필요한 것을 함께 준다 — `id` · `disambig`(기존 원장) · `qualifier`(공통 원장) ·
`entity_type` · `note`. 맥락으로 좁히려면 `disambiguate: true` 를 쓴다.

**공통 원장은 `qualifier` 가 그 자리다.** 중앙동이 50건이면 각각
`인천 제물포구` · `경남 창원시 성산구` · `전남 여수시` 를 달고 나온다 — 어느 것인지 보고
`id` 를 고르면 된다. (496,539건에 채워져 있다.)
