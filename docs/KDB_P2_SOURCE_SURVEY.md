...
| 처분 | 건수 | 비율 |
|---|---|---|
| `include` | **384,693** | 71.73% |
| `conditional` | **151,629** | 28.27% |
| `exclude_operational` | 7 | 0.00% |

(구분값 규칙 보완 후. 보완 전에는 384,676 / 151,646 이었고, 광역자치단체 17건이 풀렸다.)
...
| `no_disambiguator_cannot_be_built` | **1,010** | 원천 구분자도 없고 주소·코드로도 만들 수 없다. **지어내지 않는다** |
...
표본 검수에서 `admin_region` 의 서울특별시·경기도·세종특별자치시 등 17건이 구분값을 만들지
못해 보류됐다. 원인은 규칙이 **상위 행정구역 코드**를 쓰는데 광역자치단체에는 상위가 없기
때문이다. 상위가 없는 것이지 구분할 수 없는 것이 아니다.

행정 단계 자체(`광역자치단체`)를 구분값으로 쓴다. **그 대상의 사실이므로 지어낸 값이 아니다** —
지어내지 않는다는 규칙(식별 계약 §1.2)을 어기지 않는다.

| | 보완 전 | 보완 후 |
|---|---|---|
| 전체 `include` | 384,676 | **384,693** (+17) |
| `no_disambiguator` | 1,027 | **1,010** (−17) |
| 파일럿 B `include` | 225 | **242** |
| 파일럿 B 보류 | 24 | **7** (전부 `동구`·`강동구` 이름 충돌) |

재실행 시간은 54초 → 4분 44초로 늘었다. 원본 키 표현식 인덱스를 추가한 뒤라 53만행 INSERT
에서 인덱스 유지 비용이 붙는다. 조회가 3분 넘게 걸리던 것을 1초대로 줄인 대가이며,
dry-run 은 반복 실행이 잦지 않으므로 이 교환이 맞다.
