...
| 날짜 | 바뀐 것 | 소비자가 할 일 |
|---|---|---|
| 2026-09-24 | **제안 표기가 실제로 저장되고, 빈 칸은 제안으로 채운다.** 그전엔 요청 해석에서 `suggestions` 가 빠져 하나도 저장되지 않았다. 채운 값의 provenance 는 새 이름표 `consumer-suggestion`(최하위, `verified_only` 제외) | 자기 제안과 구분하려면 `consumer-suggestion` 으로 거른다 |
| 2026-09-24 | 틀린 위키데이터 근거 약 250건을 걷고 그 근거에서 온 칸을 비웠다(원값 스냅샷). 같은 대상인데 유형이 틀린 약 140건은 유형을 고쳤다. 요청 대상 일본어·중국어 빈칸은 잠정값(`llm-provisional`)으로 채웠다 | 잠정값은 공식 값이 오면 자동 교체된다 |
| 2026-09-24 | 답하지 못하던 요청어 1,100여 건 등록·활성화(응답률 67→93%), 이후 30분마다 자동 판정. 유형 교정 약 400건 | `review`·`preparing` 으로 받은 이름은 다시 조회. `type` 필터 결과가 바뀔 수 있다 |
