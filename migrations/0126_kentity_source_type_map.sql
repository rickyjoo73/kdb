-- 0126_kentity_source_type_map — P2.02 선매핑 코드표 승격
--
-- 승인 근거: 운영자 지시 2026-09-13 — "있는 것을 제대로 통합하고, 확장에 집중해".
-- 원본은 docs/p2/p2_premap.sql 이고 내용은 동일하다(헤더와 타임아웃만 다르다).
--
-- 이 표가 정하는 것: 원천 유형/직군을 목표 코드로 **매핑할지, 보류할지, 제외할지**.
-- 원천 유형은 후보 매핑이지 승인이 아니다(분류 규칙 §1). 보류 사유는 네 가지로 갈린다 —
-- 추측·상충·비고유명사·권리. 각 매핑은 **구분값 규칙**을 필수로 갖는다: 흡수 시 구분값을
-- 만들 수 없으면 그 코드는 애초에 매핑할 수 없다(식별 계약 §1.2).
